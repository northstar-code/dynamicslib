...
def test_new_family(
    ...
    n = len(vals_dict)
    colors = px.colors.sample_colorscale(colormap, n) if n > 1 else ["rgb(255,0,0)"]
    curves = [curve1]
    j = 0
...
def gui(
    ...
    print("\t\tCompiled")
    app.run(debug=False, use_reloader=False, jupyter_mode="inline", port=port)
...
def gui2(
    ...
    def set_family_member(fam):
        ...
        aux_data = {"arclen":arclen, "smax":smax, "s0":s0, "vals":vals}
    ...
    @callback(
        ...
        Input("flip-v", "n_clicks"),
        State("aux-data", "data"),
        prevent_initial_call=True,
    )
    def flip_vert(_, aux_data):
        if aux_data is None:
            raise PreventUpdate
